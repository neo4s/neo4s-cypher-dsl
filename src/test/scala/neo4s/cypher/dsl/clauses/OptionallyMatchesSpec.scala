...
class OptionallyMatchesSpec extends AnyWordSpec with should.Matchers {
  "Matches" should {
    ...
    "provide query for a single case class" in {
      ...
      matches.toQuery() shouldBe DSLResult("OPTIONAL MATCH (person:Person {id: $person_id,name: $person_name,age: $person_age})",
    ...
    "provide query for a node instance" in {
      ...
      matches.toQuery() shouldBe DSLResult("OPTIONAL MATCH (person:Person {name: $person_name})",
                                           Map("person_name" -> person.name))
    }
    "provide query for a any node" in {
      val matches = OptionallyMatches(any[Person])
      matches.toQuery() shouldBe DSLResult("OPTIONAL MATCH (person:Person)")

    }
    "provide query for a path" in {
      val matches = OptionallyMatches(person --> department)
      matches.toQuery() shouldBe DSLResult(
        "OPTIONAL MATCH (person:Person {id: $person_id,name: $person_name,age: $person_age})-->(department:Department {id: $department_id,name: $department_name})",
