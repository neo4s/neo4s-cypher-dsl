...
private[cypher] class Path(val pathLinks: PathLink*) extends SupportsWhere {
  ...
  def toWhereQuery(context: Context = new Context()): DSLResult = {
    val (queryList, paramMap) =
      pathLinks.map(_.toWhereQuery(context)).foldLeft((List.empty[String], Map.empty[String, Any])) { (acc, result) =>
        (acc._1 :+ result.query, acc._2 ++ result.queryMap)
      }
    DSLResult(queryList.mkString, paramMap)
  }
